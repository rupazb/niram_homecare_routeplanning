...
Cleans the raw patient roster export
Splits patients into visit-frequency tiers (weekly / fortnightly / monthly) based
on mobility status
Geographically clusters patients within each tier and assigns each cluster a
fixed weekday, so the same patients are visited on the same day every cycle
On any given day, calls a route optimizer to work out the best order to visit
that day's patients, starting and ending at the hospital
Displays everything on an interactive map with a sidebar showing the ordered
...
Project lat/long to a flat x/y plane in kilometers (longitude scaled by
`cos(latitude)` so distances aren't distorted — fine for a ~50km span).
Run standard k-means to get initial cluster centers.
Re-assign patients using a greedy capacity-constrained pass: every
...
Recompute cluster centers from the new assignment and repeat the constrained
pass a few times to let geography re-settle within the balance constraint.
Clusters are labeled with actual weekdays in west-to-east order (by cluster
...
Optimization endpoint (`/optimization`, VROOM engine) — given the
...
Directions endpoint (`/v2/directions/driving-car/geojson`) — takes that
...
---
6. Setup
Put the patient roster into a Google Sheet (a tab named `Patients`, with
...
In Google Cloud Console, enable the Google Sheets API on a project and
...
Deploy `google_apps_script.gs` on the same Sheet as a Web App (steps are in
...
Get an OpenRouteService API key (free tier covers this scale) and paste it
into `ORS_API_KEY` near the top of `niram_route_dashboard_v3.html`.
In `prepare_patient_routes.py`, set `SHEET_CSV_EXPORT_URL` to the Sheet's
...
Place `hospital_depot.geojson` and `niram_route_dashboard_v3.html` together
